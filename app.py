...
class SesionPairProgramming:
    ...
    def conectar_usuario(self, usuario):
        if len(self.participantes) >= self.limite_usuarios:
            return False
    ...
    def verificar_sincronizacion_archivo(self):
        if not self.realtime_sync_active or len(self.archivos_abiertos) < 2:
            return False
        archivos = list(self.archivos_abiertos.values())
        return archivos.count(archivos[0]) == len(archivos)
...
def abrir_archivo(task_id):
    ...
    data = request.json
    usuario = data.get("usuario", "").strip()
    nombre_archivo = data.get("archivo", "").strip()
